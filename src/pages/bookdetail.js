            ...
            if (book.price !== "0.00") {
                const purchaseResponse = await fetch(`http://localhost:3001/api/purchases`, {
                    ...
                    body: JSON.stringify({
                        ...
                        sellerId,
                        status: 'pending', // or any other default status if needed
                    }),
                });

                if (!purchaseResponse.ok) throw new Error('Failed to log purchase');
                console.log('Purchase logged:', await purchaseResponse.json());
            } else {
                // If it's a trade
                ...
                console.log('Trade created:', await response.json());
            }
