...
function Dashboard() {
    ...
    useEffect(() => {
        // Fetch book counts when the component mounts
        fetch('http://localhost:3001/api/book-counts') // Full URL
            .then(response => {
                if (!response.ok) {
            ...
            .catch(error => {
                ...
                setBookCounts({ selling: 0, trading: 0 }); // Set counts to zero on error
            });
    }, []);
