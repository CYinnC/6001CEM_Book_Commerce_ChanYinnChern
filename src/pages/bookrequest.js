...
function Bookrequest() {
    ...
    const handleAcceptRequest = () => {
        if (!selectedRequest) return; // Safeguard against null
    
        const buyer = buyerDetails[selectedRequest.buyer_id];
        const seller = sellerDetails[selectedRequest.seller_id];
        const sellerBook = bookDetails[selectedRequest.id]; // Assuming this is the seller's book
        const buyerBook = selectedRequest.title; // Set buyer's book to the title from selectedRequest
    
        // Use request.buyer_id and request.seller_id directly
        ...
        doc.setFont("normal");
        doc.addImage(Logo, 'PNG', 70, 10, 19, 15); 
        ...
        doc.text(`The seller has agreed to trade the book "${sellerBook.title}" with the Buyer's book "${buyerBook}"`, 20, 140);
        doc.text(`at the meet up location: ${selectedRequest.meet_up_location}.`, 20, 150);
        
        doc.text(`All trades are made final and cannot be changed once the agreement is made.`, 20, 170);
        doc.text(`Date of agreement: ${currentDate}`, 20, 180);
            ...
            <div className="container">
                ...
                <div className={`sidebar ${isSidebarOpen ? "open" : ""}`}>
                    ...
                    <ul>
                        ...
                        <li><Link to="/pages/recommendbook">Recomanded Book</Link></li>
